...
def get_result(request):
    """
    result.html から post request が飛んできたらモデルに保存してあるlossの値を表示する
    result.html から get request が飛んで来たら、caluculations.py のresults を動かして結果を作成し、ダウンロードさせる 
    """
    if request.method =="POST":
